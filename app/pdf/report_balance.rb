  ...
  def body
    ...
    @sales.each do |sale|
      ...
      table([[sale.invoice_number.id, sale.client.name, 
        ...
        "$#{number_to_currency(sale.remaining_amount, :format => "%u%n", :unit => '',:delimiter => ',',:separator => '.')}", sale.payments.last.created_at.strftime("%d/%m/%Y")]], :column_widths => [100,150,100,100,100])
